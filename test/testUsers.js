...
// Import test data
const { alice, emptyName, emptyUsername, emptyEmail, emptyPassword, invalidEmail, invalidPassword, invalidName, invalidUsername } = require('./data');

let refresh_token = '';
let access_token = '';

describe('/users', () => {
    // Resets database before starting this section of tests
    before(function () {
        shell.exec('./scripts/db_reset.sh');
    });
            ...
            describe('Fails to register', () => {
                ...
                it('User with all empty fields', done => {
                    server
                        ...
                        .end((err, res) => {
                            res.should.have.status(400);
                            res.body.should.be.equal('Empty field(s): username, name, email and password required');
                            res.body.should.be.a('string');
                            done();
                        });
                });
                ...
                it('All fields empty', done => {
                    server
                        ...
                        .end((err, res) => {
                            res.should.have.status(400);
                            res.body.should.be.a('string');
                            res.body.should.be.equal('Empty field(s): username and password required');
                            done();
                        });
                });
